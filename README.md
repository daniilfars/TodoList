...
- **JWT + HttpOnly Cookies** — безопасная аутентификация с защитой от XSS-атак
- **BCrypt.Net-Next** — надежное хеширование паролей
- **Swagger / OpenAPI** — интерактивная документация API
- **System.Linq.Dynamic.Core** — динамическая сортировка данных
- **xUnit & FluentAssertions** — профессиональный стек для тестирования
...
## Тестирование(написано для первоначальной версии проекта, для текущей версии тесты не работают)

Проект следует принципам надежности и покрыт Unit- и Integration-тестами.

### Инструменты
- **xUnit** — основной фреймворк для написания тестов.
- **Moq** + **MockQueryable.Moq** — имитация зависимостей и работа с `IQueryable` (DbSet) в памяти.
- **FluentAssertions** — использование человекочитаемых проверок (например, `result.Should().Be(200)`).
- **WebApplicationFactory (ApiFactory)** — кастомная фабрика для поднятия API в памяти при интеграционном тестировании.
- **InMemoryDatabase Isolation** — использование уникальных имен БД (`Guid.NewGuid()`) для каждого экземпляра теста, что гарантирует полную изоляцию и стабильность при параллельном запуске.

### Что покрыто тестами
- **AuthService & UserService**: бизнес-логика регистрации, логина и обработки ошибок (дубликаты email, неверные пароли).
- **AuthController**: интеграционные тесты эндпоинтов `/login` и `/logout`. Проверка корректной установки и удаления `Set-Cookie` (флаги `HttpOnly`, `Expires`).
- **ProjectController**: интеграционные тесты CRUD-операций (Create, Delete) с проверкой реального состояния базы данных после запросов.
- **Security**: проверка работы атрибутов `[Authorize]` и корректности обработки токенов, извлеченных из кук.

### Запуск тестов
Для запуска всех тестов из корня решения выполните:
```bash
dotnet test
```
...
>  **Важно:** Токены аутентификации (`accessToken` и `refreshToken`) устанавливаются автоматически в **httpOnly cookies** и не возвращаются в теле ответа. Браузер сам будет отправлять их при каждом запросе.
...
Пользователь может редактировать и удалять только свою учётную запись.
...
└── 🧪 5. Infrastructure.UnitTests (Проект тестов, дорабатывается)
    └── Services/          # Модульные тесты сервисов с использованием Moq
    └── Helpers/           # ApiFactory с изолированной InMemory БД
    └── Controllers/       # Тесты эндпоинтов Auth и Project
```
...
> **Для curl с cookies нужна дополнительная настройка. Рекомендуется использовать Postman или Swagger для тестирования.**
